...
namespace Naninovel.UI
{
    public abstract class TextRevealBehaviour
    {
        ...
        public void SetRevealProgress (float revealProgress)
        {
            ...
            if (revealProgress <= 0) { HideAll(); return; }
        ...
        public void RevealNextChars (int count, float duration, CancellationToken cancellationToken)
        {
        ...
        {
            if (graphic.transform.position != positionLastFrame)
            {
                UpdateClipRects();
                Update();
            }
        ...
        private void UpdateRevealState ()
        {
            ...
            if (rebuildPending && !revealState.CancellationToken.CancelASAP) return;
        ...
        private void SetLastRevealedCharIndex (int charIndex)
        {
            ...
            if (!rightToLeft && charIndex > 0 && nextChar.LineIndex == curChar.LineIndex && charIndex > LastRevealedCharIndex)
                charIndex = SkipBackwardCharacters(curChar, nextChar, charIndex);
        ...
        private int SkipBackwardCharacters (RevealableCharacter curChar, RevealableCharacter nextChar, int charIndex)
        {
