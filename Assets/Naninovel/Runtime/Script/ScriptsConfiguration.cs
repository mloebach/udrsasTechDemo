...
namespace Naninovel
{
    ...
    public class ScriptsConfiguration : Configuration
    {
        ...
        [Tooltip("Whether to run a file system watcher over `.nani` files in the project. Required to register script changes when edited with an external application. Restart Unity editor for changes to take effect.")]
        public bool WatchScripts = true;
        [Tooltip("When `Watch Scripts` is enabled, select a specific directory to watch instead of the whole project to reduce CPU usage. Restart Unity editor for changes to take effect.")]
