...
Shader "Naninovel/RevealableTMProText RTL" {
...
uniform float		_PerspectiveFilter;
        ...
        c *= IsPositionOutsideRect(input.worldPos.xy, _LineClipRect);
        // Apply gradient fade to the current line.
        if (input.worldPos.y < _CharClipRect.w)
        {
            static const float PI = 3.14159;
            float alpha = PI / 2.0 - radians(_CharSlantAngle);
			float distance = (input.worldPos.x - _CharClipRect.z) + (input.worldPos.y - _CharClipRect.y) / tan(alpha);
            c *= saturate(distance / _CharFadeWidth);
        }
        // ---------------------------------------------
